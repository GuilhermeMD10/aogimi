...
- **A component that genuinely needs a filled surface gets its own token group, not a fill of the shared ones.** `--deck-*` (`features/study/decks`) is the precedent: the deck card is one clipped object with a dark panel over paper and can't be shadow-separated, but filling `--card`/`--cardalt`/`--bd` to fix it would repaint every finished screen. The app-wide switch to filled cards is still one line in `ds-tokens.css` and still unused.
...
- **There are no pinned design canvases any more.** `aogimi-DS/` and `HomeDemos.tsx` are deleted; `BooksDesk.tsx` and `DictionarySidekick.tsx` are live production screens awaiting redesign, not reference layouts (`DictionaryQuiet.tsx` is gone — the dictionary has been redesigned). Every screen gets replaced eventually — a redesign is expected to rewrite the screen it's assigned, and there's no need to preserve the outgoing one's inline pixel radii.
