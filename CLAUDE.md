...
- **A component that genuinely needs a filled surface gets its own token group, not a fill of the shared ones.** `--paper-*` is the precedent (born as `--deck-*`, generalized when profile became the second consumer; settings is the third): ruled lists and the deck card need a real fill and edge, but filling `--card`/`--cardalt`/`--bd` to fix them would repaint every finished screen. `shared/components/PaperCard.tsx` is the ruled-list shell that reads the group. The app-wide switch to filled cards is still one line in `ds-tokens.css` and still unused.
