...
| `web-frontend/shirube-web/` | Next.js App Router web app | Next 16, React 19, Tailwind v4, shadcn |
| `mobile-frontend/shirube-mobile/` | Expo mobile app (iOS, Android, web target unused) | Expo 55, React Native 0.83 (Fabric) |
...
### Web frontend (`web-frontend/shirube-web/`)
...
### Mobile frontend (`mobile-frontend/shirube-mobile/`)
...
3. **Whole-screen swaps** via the registry in `web-frontend/shirube-web/themes/index.ts`, resolved by `useThemedComponent` (`web-frontend/shirube-web/themes/useThemedComponent.ts`). Variants live at `themes/<theme>/<mirror-of-components-path>/<X>.tsx`. **Last resort** — only for screens whose visual *tree* genuinely differs.
...
Full docs:
- `web-frontend/shirube-web/PROJECT_CONTEXT.md` — start here, end-to-end overview.
- `web-frontend/shirube-web/THEMES.md` — token inventory + per-token consumer list + dispatch decision rule.
- `web-frontend/shirube-web/THEME_AUTHORING.md` — step-by-step playbook for adding a theme.
- `web-frontend/shirube-web/backend-connections.txt` — endpoint catalog + payload shapes the frontend depends on.
- `web-frontend/shirube-web/DECISIONS.md` — scope decisions + deferred work.

### Web: state architecture

Six React Context providers in `web-frontend/shirube-web/components/providers/`, mounted by `AppShell`. **No Redux, Zustand, Jotai, etc.** Cross-feature signalling happens through "pending fields" on `ReaderStateProvider` — e.g. the reader sets `pendingDictSearch`, `AppShell`'s effect picks it up, calls `dict.runSearch`, and nulls the field. Effects are guarded against double-fire with `useRef`s of the last-handled trigger object (see `components/AppShell.tsx`).

Domain types are in `lib/types/` (NOT in `lib/<x>Api.ts` files — those only export fetch helpers).

### Web: app-level features

Documented in the **Features** section of `web-frontend/shirube-web/PROJECT_CONTEXT.md`. When a new app-level feature lands (something a user can name — "shortcuts", "highlights sync", "deck import", …), add a subsection there: what it is, entry-point files, where state lives, any non-obvious behaviour. Keep entries terse; deep details belong in the source.
...
`mobile-frontend/shirube-mobile/THEMES.md` is the mobile mirror of the web `THEMES.md`.
...
- **Next.js 16 has breaking changes.** `web-frontend/shirube-web/AGENTS.md` says: read `node_modules/next/dist/docs/` before writing route code. Conventions don't match training data — heed deprecation notices.
...
- **Two design canvases vs production**: `web-frontend/shirube-web/langeco-DS/` and `components/home/HomeView/HomeDemos.tsx` + `components/library/LibraryDesk.tsx` + `components/views/DictionaryView/{DictionaryQuiet,DictionarySidekick}.tsx` are intentionally pinned reference layouts. They use inline pixel radii on purpose — don't sweep them into the token system without explicit visual review.

## Conventions

- Domain types in `web-frontend/shirube-web/lib/types/` (one file per domain). API files in `lib/<domain>Api.ts` import their types from there; no type declarations alongside fetch helpers.
