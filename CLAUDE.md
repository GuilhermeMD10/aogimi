...
`jp-eco` (codename "Langeco") is a Japanese-reading-and-vocab ecosystem with three independently-deployable parts in this monorepo:

| Path | What | Stack |
|---|---|---|
| `backend/` | REST API for users, books, decks, devices, dictionary search | Node 18+, Express, PostgreSQL |
| `web-frontend/langecko-web/` | Next.js App Router web app | Next 16, React 19, Tailwind v4, shadcn |
| `mobile-frontend/langecko-mobile/` | Expo mobile app (iOS, Android, web target unused) | Expo 55, React Native 0.83 (Fabric) |
...
CORS expects the frontend at `http://localhost:3001` or `http://localhost:3002` by default. Set `CORS_ORIGIN` (comma-separated) to change.

### Web frontend (`web-frontend/langecko-web/`)

```bash
cd web-frontend/langecko-web
...
### Mobile frontend (`mobile-frontend/langecko-mobile/`)
...
3. **Whole-screen swaps** via the registry in `web-frontend/langecko-web/themes/index.ts`, resolved by `useThemedComponent` (`web-frontend/langecko-web/themes/useThemedComponent.ts`). Variants live at `themes/<theme>/<mirror-of-components-path>/<X>.tsx`. **Last resort** — only for screens whose visual *tree* genuinely differs.
...
Full docs:
- `web-frontend/langecko-web/PROJECT_CONTEXT.md` — start here, end-to-end overview.
- `web-frontend/langecko-web/THEMES.md` — token inventory + per-token consumer list + dispatch decision rule.
- `web-frontend/langecko-web/THEME_AUTHORING.md` — step-by-step playbook for adding a theme.
- `web-frontend/langecko-web/backend-connections.txt` — endpoint catalog + payload shapes the frontend depends on.
- `web-frontend/langecko-web/DECISIONS.md` — scope decisions + deferred work.

### Web: state architecture

Six React Context providers in `web-frontend/langecko-web/components/providers/`, mounted by `AppShell`. **No Redux, Zustand, Jotai, etc.** Cross-feature signalling happens through "pending fields" on `ReaderStateProvider` — e.g. the reader sets `pendingDictSearch`, `AppShell`'s effect picks it up, calls `dict.runSearch`, and nulls the field. Effects are guarded against double-fire with `useRef`s of the last-handled trigger object (see `components/AppShell.tsx`).

Domain types are in `lib/types/` (NOT in `lib/<x>Api.ts` files — those only export fetch helpers).

### Web: app-level features

Documented in the **Features** section of `web-frontend/langecko-web/PROJECT_CONTEXT.md`. When a new app-level feature lands (something a user can name — "shortcuts", "highlights sync", "deck import", …), add a subsection there: what it is, entry-point files, where state lives, any non-obvious behaviour. Keep entries terse; deep details belong in the source.
...
`mobile-frontend/langecko-mobile/THEMES.md` is the mobile mirror of the web `THEMES.md`.

### Books: per-device storage + Postgres metadata

EPUB/PDF blobs never go to Postgres. They live in IndexedDB (web) or local FS (mobile, planned). The backend stores **only metadata + reading progress** in `book_progress` rows.
...
- **Next.js 16 has breaking changes.** `web-frontend/langecko-web/AGENTS.md` says: read `node_modules/next/dist/docs/` before writing route code. Conventions don't match training data — heed deprecation notices.
...
- **Two design canvases vs production**: `web-frontend/langecko-web/langeco-DS/` and `components/home/HomeView/HomeDemos.tsx` + `components/library/LibraryDesk.tsx` + `components/views/DictionaryView/{DictionaryQuiet,DictionarySidekick}.tsx` are intentionally pinned reference layouts. They use inline pixel radii on purpose — don't sweep them into the token system without explicit visual review.

## Conventions

- Domain types in `web-frontend/langecko-web/lib/types/` (one file per domain). API files in `lib/<domain>Api.ts` import their types from there; no type declarations alongside fetch helpers.
