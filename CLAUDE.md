...
- Type tokens are `--face-jp` / `--face-ui` / `--face-mono` (M PLUS 1 + Space Mono). **Not** `--font-*`, which is what `next/font` emits (`--font-mplus1`, `--font-space-mono`) — a role shouldn't read as a font variable. **M PLUS 1 ships weights 500 and 700 only**, so use `font-medium` / `font-bold`; a `font-semibold` is synthesised.
