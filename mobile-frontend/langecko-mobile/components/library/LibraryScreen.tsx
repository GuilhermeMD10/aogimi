  ...
  return (
    <Screen padded>
      <View style={styles.header}>
        <Text style={[styles.title, { color: c.fg }]}>{t('library.title')}</Text>
        <Pressable
        ...
        >
          ...
          {hero && (
            <View style={{ marginBottom: spacing.xl }}>
              <ContinueReadingCard book={hero} onPress={() => openBook(hero.id)} />
            </View>
          )}

          {books.length > 0 && (
            <>
              <Text style={[styles.section, { color: c.fgMuted }]}>Your library</Text>
              <View style={styles.grid}>
                {books.map((b) => (
                  <View key={b.id} style={styles.gridItem}>
                    <BookGridItem book={b} onPress={() => openBook(b.id)} />
                  </View>
                ))}
              </View>
            </>
          ...
          {books.length === 0 && !error && (
            <View style={styles.emptyWrap}>
              <Text style={[styles.empty, { color: c.fgMuted }]}>{t('library.empty')}</Text>
            </View>
          )}
        </ScrollView>
      )}
