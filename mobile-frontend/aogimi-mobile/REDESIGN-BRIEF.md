...
| jlpt | **Superseded 2026-09-16 (owner):** not in the palette. `JLPT_CHIP` is a fixed, theme-independent map — N5 blue · N4 green · N3 yellow · N2 orange · N1 red, each an opaque `inner` fill + `outer` border/label. Set in stone; never re-add `jlptN*`. | — |
...
| 2026-09-16 | **sky/stage** | Sky root (outer + focused tiers), deck menu, deck stats sheet, edit-deck sheet, star inspector drawer, **cards list** (`/sky/[deckId]`, new), New-deck sheet re-skinned | **Theme:** `ThemeScope` in `theme/ThemeContext.tsx` — pins a subtree to one column; the whole sky sits in `ThemeScope name="night"`, which is what retired `stage/lib/nightChrome.ts` (the stage now renders inside `Screen`). **Primitives:** `Header` (60pt row, fixed 44/36 end slots, JP title + mono subtitle, built-in back/more, `right` slot), `PopoverMenu` (Modal, scrim + blur 8, Tier 4 280pt R16, 56pt rows w/ 40pt plate, accent/destructive/meta, `header` + `caption` slots; **row actions fire from the Modal's `onDismiss` on iOS** so a sheet can follow a menu), `StatTile`, `StateTag` (`sm` 10px caps / `md` 14px; tone from caller), `MeaningRow`, `SearchField` (48pt Tier 1, JP placeholder, clear refocuses). `IconButton` gained a `children` slot (the green cloud-sync SVG). **Renderer (additive, `map/components` only — `map/lib` untouched):** `SkyCanvas` gained a `Gesture.LongPress` (420ms, outer tier only) → `onLongPressDeck(did)`; `SkyMap` threads it as `onLongPressDeck(deckKey)` and mounts new `SkyDeckBadges` — the due badge at each deck's star-box top-right, riding the live camera via `useAnimatedStyle`, first consumer of `frameMeta.dueCount`. **Stage lib:** `deckStats.ts` (`recentRetentionOf` from `last_outcomes`, `avgIntervalDaysOf` from `stability`, `lastSevenDays`). **i18n:** new `sky.*` namespace ×3 locales. **Deleted:** `StageLedger`, `StageActions`, `SkyDeckBar`, `CardDetailSheet`, `DeckDetailScreen`, `CardGridItem`, `DeckCover`, `nightChrome.ts`. | 29 | Sky is **Night in both themes by construction** (`ThemeScope`) — the handoff's Day variants of the menu/stats/inspector were not built; removing the scope is the one-line reversal if the owner wants them. Retention is *recent* (last ≤5 grades per card), avg interval is the *scheduled* one; 7-day activity is **account-wide** (`/api/stats/activity` has no `deckId` filter — backend gap). **Dropped from the old deck screen and not re-homed:** add-card from a deck (`FlashcardDrawer` still used by reader/dictionary), per-card edit (`CardEditSheet` now orphaned), per-deck session config (`SessionConfigSheet` orphaned; `useDeckOverrides` still read by the study route), `StateBreakdown` (study's) orphaned. Cards-list header `…` is an inert placeholder by the owner's note. `StageActions`' pending-sync border cue was not carried. `SkyCanvas`/`SkyStars` still carry the owner's uncommitted label edits — the long-press is layered on top, not a rewrite. Next: sky/study. |
| 2026-09-16 | **dictionary** | Dictionary tab (empty / results / entry), lookup sheet (`DictDrawer`, consumed by reader, study, sky) | **Primitives extended:** `IconButton` gained `size={40}` (16pt glyph — DESIGN.md's dictionary add circle); `MeaningRow` gained `meta` (mono line under the gloss, for a per-sense POS). **Feature-local:** `ResultsKicker` (`RESULTS · 8 for · 「じしょ」`, shared by tab and sheet); `AddButton` is now a 40pt `IconButton` (Tier 1, accent on the leading row); `MetaChip` is the 6px borderless POS tag; the feature's `SearchField` re-cut onto `InnerPlate` (48pt R12, faint glyph) **keeping its full-height clear segment and clear→focus rule by the owner's instruction** — not swapped for `shared/SearchField`. Every dictionary file now on `usePalette()` + `type.*`; no `fontFamily`/`fontSize`/legacy radius left in the feature. **Deleted:** `SuggestionChips`. | 29 | **Owner to rule on:** (1) the three seed chips under the field were removed because the `DictionarySearch` composition has none — restore = re-add the component and its `onPickSuggestion` prop; (2) the composition puts the hero *above* the field, the tab keeps the field pinned at the top with the hero under it so the field never moves or remounts while typing (the keyboard rule the owner asked to keep) — flag if the order matters; (3) the recents composition draws a 40pt add circle per row; **not built** — the `RecentLookup` snapshot has no meanings, so adding would need a fetch first (cheap: `peekWord`/`fetchWordDetails` by `wordId`) — the slot carries the time-ago instead, per DESIGN.md's row spec. **Entry page (no handoff):** search bar removed on the detail frame per the owner's note — the pinned block is a 44pt back circle there; `useDictionaryNav` unchanged. Entry re-skinned, layout kept: primary `Button`, `MeaningRow` plates, kanji `Card`s, `displayKanji`/`displayKanjiMobile` headword. **Sheet:** height 0.65 → 0.60 (Reader composition), `stackGap` rhythm, RESULTS kicker added; the `compact` step-down was dropped from the rows, field and list because the pop-up composition draws them at the tab's size (kept on `EntryView` only). `PitchAccentDiagram` and `RubyText` in `shared/` still read `useColors()` — not this feature's files, left alone. `BackButton` has no dictionary caller left (settings/profile still use it). **Owner ruling, same day:** JLPT chip colours are fixed in both themes — `jlptN1..5` removed from both palette columns, `JLPT_CHIP` (two opaque tones per level) added to `tokens.ts`, `Tag` gained `fill`/`border` overrides, `JlptChip` no longer reads the palette. |
