...
> **This directory is a verbatim copy of `web-frontend/aogimi-web/features/sky/map/lib`.**
> Not "based on" — byte-identical, deliberately. Edit one, copy it to the other, and run
> `npm run verify:sky` in this package: it compiles both copies and asserts every star position,
> link and constellation matches, plus golden values recorded independently of either copy.
> A silent drift here means a user's star sits in a different place on their phone than on the
> web, which is the one thing the whole feature promises not to do.
>
> The mapping from a `CardRecord` to what this engine consumes lives *outside* it, in
> `features/sky/lib/skyProjection.ts` — that is what keeps this directory free of FSRS, decks and
> the API.
