...
Expo + React Native companion to the Shirube web app. iOS + Android
...
- **iOS device install hang with `expo run:ios`** — known
  ...
  Workaround: open `ios/Shirube.xcworkspace` in Xcode and Cmd+R
...
`langecko-mobile` → `shirube-mobile`. Some `lgc_*` AsyncStorage keys
