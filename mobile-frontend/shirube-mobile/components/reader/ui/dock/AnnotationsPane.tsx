...
/** Pure annotations content — header + tabs + scrollable list. The dock
 *  provides the surrounding frame and handle. */
export function AnnotationsPane({
  bookmarks,
  highlights,
  onJumpBookmark,
  onDeleteBookmark,
    ...
    <View style={styles.root}>
      ...
      <View style={[styles.tabs, { borderBottomColor: c.border }]}>
        {(['bookmarks', 'highlights'] as Tab[]).map((t) => (
          ...
          >
            <Text style={[styles.tabLabel, { color: tab === t ? c.fg : c.fgMuted }]}>
              {t === 'bookmarks'
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {tab === 'bookmarks' &&
          (bookmarks.length === 0 ? (
            <Text style={[styles.empty, { color: c.fgMuted }]}>No bookmarks yet</Text>
          ) : (
            bookmarks.map((b) => (
              <Row
                key={b.id}
                onJump={() => onJumpBookmark(b)}
                onDelete={() => onDeleteBookmark(b.id)}
              >
                <Text numberOfLines={1} style={[styles.rowLabel, { color: c.fg }]}>
                  {b.label}
                </Text>
              </Row>
            ))
        ...
        {tab === 'highlights' &&
          (highlights.length === 0 ? (
            <Text style={[styles.empty, { color: c.fgMuted }]}>No highlights yet</Text>
          ) : (
            highlights.map((h) => (
              <Row
                key={h.id}
              ...
              >
                <View
                  style={[styles.swatch, { backgroundColor: HIGHLIGHT_COLORS[h.color] }]}
                />
                <Text
                  numberOfLines={2}
...
const styles = StyleSheet.create({
  ...
  empty: { fontSize: fontSize.sm, padding: spacing.xl, textAlign: 'center' },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 12,
