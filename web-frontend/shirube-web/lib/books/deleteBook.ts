...
import { deleteBook as deleteLocalBook } from '@/lib/bookStore';
import { deleteBookRecord } from '@/lib/booksApi';
 ...
 * locally), so it's safe to call on any LibraryBook.
 */
export async function deleteBookEverywhere(book: LibraryBook): Promise<void> {
  if (book.backendId) {
