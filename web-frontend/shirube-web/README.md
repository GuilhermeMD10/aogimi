...
Next.js 16 App Router web app for the Shirube ecosystem. Same backend
...
- **Two design canvases vs production**: `langeco-DS/` and
...
The project was called "Langeco" / "langecko" earlier — the CSS
variable prefix is still `--lgc-*`, the localStorage prefix is a mix
of `lgc_*` and `shirube_*`, and the design canvas folder is
`langeco-DS/`. These are stable identifiers, not branding.
