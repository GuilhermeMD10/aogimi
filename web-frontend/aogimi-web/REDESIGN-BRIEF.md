...
- **`design-handoff/2026-09-21-web/`** — the designs. **Removed 2026-09-22**, once
  every page it drew had landed; the values it carried live on in
  `styles/ds-tokens.css`, the primitives and this brief. `README.md` there was the
...
| **D11 Footer** | **Deleted from every page** (owner, 2026-09-22). No `Footer` primitive, no footer column in `frameForRoute`, no `--pane-footer`. The handoff's footer regions (pages 01 · 02 · 08) and G13 are moot. |
...
| `Footer` | **removed (D11)** — built in Phase 1, deleted 2026-09-22 | — |
...
| 2026-09-22 | **dictionary** | 02 · 03 (rows + field for 10) | **Tokens:** `--pane-entry` (page 03's 135° soft-accent → white entry-card gradient; glass stops on Night). **Primitives:** `Chip` (README's POS chip, R6 / `LINE .05` / 10px 500 uppercase; `dot` prop makes it the state chip), `Button` gained `variant="good"` (R12 `--good` fill, `--good-glow`, `--on-accent` label). **Shell:** `FrameOverrideProvider` + `useFrameOverride` (`features/app-shell/providers`) — a mounted page lays fields over its `frameForRoute` row; `SearchView` widens `/dictionary` to `wide` while results show, so the sticky-query state follows what is on screen rather than the URL. **Dictionary:** `BeforeSearch` rewritten to page 02 (tag pill · 48/700 headline · italic subtitle · 560 hero `SearchField` with `Enter` pill · suggestions = newest recents · `RecentLookups` R20 rows with `Clear log`); `SearchView` + `ResultsRail` to page 03 (`316px minmax(0,1fr)` grid, R14 query field, `RESULTS · N FOR 「q」` caption, cards with the `good` ring + `good` `+` circle, entry card R28 on `--pane-entry`; stacks below `lg`, where `‹ back to results` appears); `ResultRow` at two scales (`compact` 316px/docked · `full` modal with the tinted primary `+`), `RailList` takes `scale`; `EntryDetail` to page 03 (`EntryHeader` `DICTIONARY · 辞書`, 76/700 headword, JP 22 reading, JLPT · POS `Chip` · `good` kanji-grade chips · pitch in the chip row, `Add to deck` good, numbered meanings, 2-col `KanjiCard`s, ruled examples with grade chip); `KanjiEntryDetail` re-skinned to match (no handoff); `SectionLabel` draws its Japanese half again; `PitchAccent` recoloured; `SearchField`'s three variants restyled (hero 58 pill · rail 48 R14 · sidebar 48 R12), typed text in the JP face; `useRecentSearches` gained `clear`. | `Constellation.tsx`; the rail's brand mark; both `pb-[120px]` Dock reserves; the entry source lines (`Source · JMdict` — attribution lives on `/credits`); `ROW_*`/`ClassPill`/`AddButton`/`WordRow`/`KanjiRow` barrel exports (no consumer outside the feature); the `text-muted-foreground` alias uses. | **Owner rulings:** recents stay strings, a row re-runs the query (G4); bookmark cut (G5); suggestions = recents, hidden when none; inner scroll kept; `+` → check cut; the footer is gone from every page (D11). **Not built:** entry-shaped recent rows, bookmark, `+` → check, the 1.5px selected border (1px + ring, so selecting never shifts layout), a POS label mapper (rows print the raw JMdict `pos` as today). **Outside the feature:** `FrameOverrideProvider` mounted in `AppShell`, merged in `AppFrame`; `dict-sidebar/index.tsx` passes `scale="compact"` to `RailList` (one line). `SectionCard` / `MeaningRow` were **not** added — neither page 02 nor 03 uses them (03's meanings are a plain numbered list; its cards are bespoke); they land with the first screen that draws one (study). `--theme-name` + `THEMES[].name` are now unconsumed (the footer was their reader). Night derived and unverified in a browser; nothing was eyeballed this session. |
| 2026-09-22 | **sky/stage** | 04 · 05 | **Tokens:** `--field-panel-bd` (the inspector's `rgba(255,255,255,.18)` edge) in the `:root` field block. **Primitives:** `MeaningRow` (`pane` = the README's numbered row, `field` = the inspector's night variant — study's back gets `pane` for free), `SearchBar` gained `size="sm"` (44px, the card list's filter pill). **Backend:** `next_due_at` added to the `view=sky` projection (`cardRepository.findSkyByDeckIds`; `API_ROUTES.md` + `backend-connections.txt` updated), `SkyCardRecord`/`toSkyCard` carry it. **Frame:** `/sky` stays `flow: 'fill'` (owner: the field fills the viewport, the list scrolls beside it — pages 04/05's 960/880 field on a scrolling page is not built). **Stage:** `SkyView` rewritten to pages 04/05: the map inside the **Sky field** (viewport-high, `rounded-(--radius-hero)`, `--field-bd`, the nebula + aurora gradients over `--field-bg`/`--field-bg-deck`; nothing moves when a deck or card opens), camera insets field-relative (`FIELD_TOP` 92 · 24 gutters · 360 right while the inspector is open); `FieldHeader` (eyebrow + `N constellations` caption or back circle + deck name + description · stars pill · `⋯` · `StudyButton`); `DeckMenu` (`⋯` popover: New deck · Rename deck · Delete deck); `DeckNameModal` (create + rename on `Modal`); `ConfirmDialog` (D9 re-skin of `NightConfirm` on `Modal`); `Inspector` (page 05 panel: JLPT + rank label, front, `[reading]`, `Interval · Next`, `MeaningRow`s, context sentence with the word lit, View in dictionary + delete); `CardsSection` (a 340px column beside the field, both tiers: title + `shown / total`, full-width SearchBar, a one-line sideways-scrolling row of filter chips `All · Due · New · Met · Learned · Mastered` with counts, Added · Mastery · JLPT sort chips, 56px rows with rank `Chip`, interval cell, deck name at the sky tier, selected tint, scrolling inside the column; skeleton and no-match states — an empty scope draws nothing, the field's own empty line covers it); `StudyButton` on `Button` (`Continue Studying · N DUE` / `Study Deck Due · N` / white `Study ahead`); `useSkyDecks.renameDeck` patch; `lib/cardLabels.ts` (`isCardDue`, `intervalLabel`, `nextDueLabel`, `startedLabel`, `addedLabel`), `rankProgress.rankArgs`. `PracticeOverlay` + `PendingCardOverlay` are `fixed` now (the page scrolls) and the pending scrim reads `--scrim`. | `nightChrome.ts`, `StageActions`, `StageLedger`, `DeckBar`, `GlassColumn` + `ColumnHandle`, `CardSearch` (the cross-deck dropdown), `CardDetailCard`, `MixBar`, `NightConfirm`, `useSkyLedger`, `masteryMix.ts`; the panel collapse; the detail card's MASTERY meter and ADDED line; the whole-sky `/api/stats/activity` fetch. | **Owner rulings:** no SYNCED, no Stats (G6/G7 cut); `⋯` holds New · Rename · Delete deck; list on both tiers (sky = every card newest first, click flies + rings; deck = its cards), search/filter only narrow; sort chips kept; `next_due_at` added, chips show rank, Due is a filter. **Judgment calls to eyeball:** the filter is six segments (the ladder + Due) rather than the handoff's three; the row's mono cell is the interval until a sort is on, then that sort's value; the filter is a one-line scrolling chip row rather than one `Segmented` pill (six items don't fit 340px); the outer caption reads `pick one to open its deck` because the chooser is locked (no zoom); `Study Deck Due` is the handoff's label verbatim. **Not built:** the field's own procedural star layer (the map's stars are the content; a decorative layer under them reads as cards), node sub-labels / orbit rings / due badges on deck nodes (frozen map, D5), star dimming on filter (no map hook), the inspector's pitch polyline / translation / source row / `⋯` (no data), the `N MORE`-style scroll-to-row on star click (the page would jump). `SkyBar` still has study + auth callers; `PaperCard`/`GlassCard` untouched. `--field-ink*` are unconsumed (the header's pills are `.pane` so Night makes them glass; the panel reads `--night-ink`). **Verify:** `tsc` 0 · lint 0/0 · build — see the session report. Night derived, unverified in a browser; nothing eyeballed this session. |
| 2026-09-22 | **auth** (`/authenticate`) | auth handoff (Sign In `#web-signin` · Sign Up `#web-signup`; not in `design-handoff/` at the time of writing — file it as `design-handoff/2026-09-22-auth/`) | **Tokens:** `--pane-auth-login` / `--pane-auth-signup` (the brand panel's 158° gradients, sakura-led / sky-led; glass-strength stops on Night). **Primitives:** `Brand` (the 38px 仰 circle + wordmark, now drawn by `TopNav` and the auth bar), `StarField` gained `count` + `className` (the panel's 26 stars are the first 26 draws of the same seeded sequence). **Auth:** `AuthView` is bar → centred `.pane` card at R28 with `--shadow-hero`, `400px minmax(0,1fr)`; `AuthTopBar` (`.pane-nav`, brand + the one-item pill that flips the mode); `BrandPanel` (gradient by mode, stars, 64px mark, headline + JP line, three pillar rows; a mark + headline strip below `lg`); `AuthField` (52px R14 `--pane-strong` box, `LINE .08` edge, 16px lucide icon that takes `--accent` on focus, the 1px + 3px-ring focus treatment, Show/Hide pill inside); `PasswordStrength` + `lib/passwordStrength.ts` (four segments, WEAK/FAIR/STRONG pinned to the real policy: anything the server rejects is WEAK); `AuthForm` (eyebrow · 38/700 title + JP · sub · fields · reserved error-plate slot · `Button` primary lg with trailing arrow · mode line); `SocialButtons` restyled to the handoff's 2-col 50px pills (still behind `SHOW_SOCIAL_AUTH = false`). | `ModeSwitch` (the radiogroup — the top-bar pill and the footer line flip the mode now), `SkyPanel` (the night panel; `SkyBar` has no auth caller left — study is its last). | **Not built, on purpose:** `Forgot password?` (no reset), `Keep me signed in` (no session-only mode), `Starting level` JLPT chips (nothing in the register payload), the Terms / Privacy checkbox (no such pages), the bar's `Help` row (`/help` is behind the sign-in gate), the page footer (D11), the `28 cards are due today` clause (signed-out has no due count), the sign-up sub `Free while you read your first book. No card needed.` (asserts a pricing model — the app's own line stays), the sign-in pillars' live figures (`14 BOOKS`, `1,420 STARS · 28 DUE` — both modes draw the sign-up set). Sign-in is **username**-keyed (AGENTS.md → Auth), so the handoff's Email field is the Username field with a user icon. Behaviour unchanged (D10): one `validate()` problem at a time → the form-level plate; registration still returns the backend's 403. The auth page renders bare (no `AppFrame`), so Night has no page-level `StarField` there — the panel's own 26 stars are the only ones; a follow-up could mount it from `AuthView` via `useTheme`. Eyeballed in all four themes at 1280 and at 900 (headless Chrome screenshots, sakura/kanagawa/clear/night, both modes, error + focus). |
| 2026-09-22 | **sky/study** | 06 · 07 · 08 | **Tokens:** `--flip-anim` (300ms; the flashcard's turn, each half `/2`) in `:root` motion. **Keyframes** (`globals.css` → Motion): `flip-out` / `flip-in` (two quarter-turns about Y with `perspective(1400px)`), `fade-out` (the reduced-motion swap). **Primitives:** `SectionCard` (R20 `--pane` on `--hairline`, `--shadow-card`, `24px 26px`, title 20/700 + mono 11 uppercase `meta`), `StatTile` (190w R16, label 10/700 → value 34/700 in `color` → mono meta). **Study:** `StudyScreen` rewritten to pages 06/07 — `SessionHeader` (44px back circle, `STUDY · Session|Practice` eyebrow with the 5px dot, deck name 26/700 JP or scope 26/700 UI; right: counter pill `12 / 40 · 28 LEFT`), `ProgressBar` 6, `Flashcard` (760w R36 on `--pane-hero`, **one fixed 460 height on both faces** — owner, 2026-09-22; a face that overflows scrolls inside the card; the top-right magnifier is a sibling button → `useReaderActions().requestDictLookup(front, context)` opens the app-global dictionary `Modal`; study hotkeys go quiet while `readerModal` is set), `useFlip` (state adjusted in render; `out` → face swap → `in`, `animationend`-driven; new card = no turn; `motion-reduce:` crossfades), `CardBody` (front: word 68/500 · `[reading]` in accent · cloze context · 48×1 divider · `Press Space to reveal the back`; back: word 60/500 · reading · divider · wrapping `MeaningRow`s at `flex 1 1 200px` or the pre-026 `back` plate · centred example with the word lit at `ACCENT_SOFT .35` R4; rank `Chip` (`displayedRank`) + `JlptChip` per the `front.jlpt` pref in the top-left corner), action row (52px Undo circle + `Reveal card` primary lg with `SPACE`), `ResultButtons` = the grade shelf (4 tiles 72h R16, `--grade-*` at .12/.18 hover/.4 edge, label left, key square right, no interval — G17), practice note as a `.pane` row. `FinishScreen` to page 08 — header (`STUDY · <deck>` JP kicker, `Session complete` 42/700, `Study again` primary on the right), `HeroCard` (`N cards reviewed` + %, bar, mono `label · N grades · N / N`; `StatTile`s CORRECT in `--grade-good` / MISSED in `--grade-again`), 2-col `SectionCard`s (Hardest cards with `N MISSES` chip · Tier changes with `● Met → ● Learned` in `--stage-*`, `↓` rows kept — D10), a third `SectionCard` for the mix (`BreakdownBar`), `CollapsibleRows` (three rows + centred `N MORE`, expands in place), `SummaryRow` (R14 pane row → `/sky?deck=&card=`), `lib/sessionStats.ts` (`gradeTotals`, `hardestCards`, `tierChanges`, `stateCounts` — the figures and the lists compute from one place). `SessionDeck` is `{ name }`. | `RankPill`, `Caption`, `UndoButton`, `shared/components/SkyBar` (last caller gone), the `BANNER_*` literals + the night banner, `PAPER_GHOST` use in study, the deck spine chip (`kamon`/`surface`/`ink` on `SessionDeck`), the 860px card + `min(380px, 60vh)` floor, the "No example sentence on this card" plate, `deckVisuals` import in `StudyView`/`PracticeOverlay`. | **Owner rulings applied:** no interval on tiles, grade hexes untouched, no source row, no EN example line, no eyebrow on the card, no minutes, magnifier → Modal built, demotions kept, exit → `/sky`, mix bar kept (third section), `Study again` beside back, flip built, `N MORE` built; the card keeps one size front and back (the handoff's grow-to-fit is not built). **Judgment calls to eyeball:** the header's `⋯` is not drawn (display prefs have no editor on web, so nothing would sit behind it); `front.deckName` has no slot (the header names the deck); the rank chip + JLPT chip sit top-left of the card, mirroring the magnifier — the handoff draws no chips there; the tier card is titled `Tier changes`, not `Tier upgrades`, because it lists demotions; its caption is `N stars brighter · M stars dimmer`; the grade tile's key square is `--pane-strong` rather than the handoff's `rgba(255,255,255,.6)` so it holds on Night; the flashcard does not blur (trap 3); the Hard tile's `--grade-hard` (`#ffd582`) is faint on the light canvases — the owner ruled the hexes stay, flagging legibility only. **Outside the feature:** `features/sky/components/PracticeOverlay.tsx` — background `--field-bg` → `--canvas` (its own comment asked for the page canvas; the field's indigo put dark ink on a dark ground on the light themes), a `max-w-[1980px] px-24` wrapper so the runner has `/study`'s column, `SessionDeck` shape. **Not eyeballed:** no dev login was available to this session (registration is closed), so nothing was rendered in a browser on any theme — Night is derived through the tokens only. `tsc` 0 · lint 0/0 · build completes. |
