...
// no test runner — which is why it is a script rather than a `.test.ts` for a
// runner this repo doesn't have. `.mts` so Node reads it as ESM without the
// package needing `"type": "module"`, which would change how Next builds.
//
// Node prints a MODULE_TYPELESS_PACKAGE_JSON warning on stderr for the
// imported `fsrs.ts` (a Next source file, so it can't take an `.mts`
// extension). It is noise — the exit code is the result.
//
// The vectors are the same ones `backend/scripts/verify-fsrs.js` asserts,
// against the same py-fsrs 6.3.1 reference. That duplication is the point:
// `fsrs.ts` and `fsrs.js` are mirrors, and the only way a drift between them
// gets caught is if both are independently pinned to the reference. Run both
...
console.log('Web FSRS-6 mirror matches the reference.');
