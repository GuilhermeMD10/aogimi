...
import { decksApi } from '@/features/sky/stage';
...
export default function StudyView() {
  ...
  // The header's deck identity — the name, resolved here from the decks layer
  // because sub-features don't import each other; the orchestrator that
  // already holds both is where the two meet.
  const deck = deckId ? decks?.find((d) => d.id === deckId) ?? null : null;
  const sessionDeck: SessionDeck | null = deck ? { name: deck.name } : null;
