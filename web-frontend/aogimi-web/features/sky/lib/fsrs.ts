// FSRS-6. Pure maths — no React, no DOM, no card rows.
//
...
// and `map` takes star brightness from its forgetting curve. Sub-features
// don't import each other, so anything all three need lives here.
//
// The layer that turns a `CardRecord` into a review is
// `study/session/lib/srs.ts`.
//
// **This is a line-for-line mirror of `backend/src/services/fsrs.js`.** Change
// one, change both, and re-run both verification harnesses
// (`backend/scripts/verify-fsrs.js` and `lib/fsrs.verify.ts`). The backend is
// the source of truth and the only thing that writes; this copy exists so the
// study screen can show the post-review state immediately rather than waiting
// on the POST.
//
