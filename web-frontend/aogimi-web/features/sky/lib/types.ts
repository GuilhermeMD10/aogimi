...
export type Star = {
  ...
  back: string;
  /**
   * Mastery rank 0..3 — what the star is *drawn as*: its colour, its silhouette, its radius, its
   * glow. Comes from the host's own SRS ladder (new/seen/learned/mastered), carried in on
   * `addStar` like the faces are. Placement never reads it, so a card climbing the ladder
   * recolours its star without moving it or any other.
   */
  mastery: number;
  count: number;
  seen: boolean;
