...
/** Grade one card. `Outcome` is FSRS's four grades — it comes from `fsrs.ts`
 *  rather than being spelled out here so this signature, the button row and the
 *  server's zod enum can't drift into three different vocabularies. */
export async function reviewCard(cardId: string, outcome: Outcome): Promise<CardRecord> {
