...
/** The four SRS ranks. Backend: `CARD_STATES` + the `cards_state_check` DB
 *  constraint (migration 024, ladder renamed in 027 — `seen` became `met`).
 *  The thresholds that produce them live in `features/sky/lib/fsrs.ts`. */
export const CARD_STATES = ['new', 'met', 'learned', 'mastered'] as const;
