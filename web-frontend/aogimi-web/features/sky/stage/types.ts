 ...
 * never from answer streaks. `met` was called `seen` before migration 027.
 *
 ...
 *   mastered  S ≥ 365
 *
 * Mirrors `fsrs.RANKS` and the `cards_state_check` constraint.
 */
export type CardState = 'new' | 'met' | 'learned' | 'mastered';
...
export interface CardRecord {
  ...
  peak_rank: CardState;
  // FSRS-6 memory state (migration 027; the columns date from 022 but their
  // meaning and scale changed completely).
   ...
   *  027 — the ladder used to read it, FSRS doesn't. `card_reviews` is the
   *  real, complete log. */
...
export type CardModel = {
  ...
  state?: CardState;
  reviewed_times?: number;
  // Needed by the deck-detail panel: `created_at` is the "Added" sort, and
  // `stability` + `peak_rank` are what `lib/rankProgress` reads to work out
  // how far a card has come toward its next tier.
  created_at?: string;
  stability?: number | null;
  peak_rank?: CardState;
  last_reviewed_at?: string | null;
