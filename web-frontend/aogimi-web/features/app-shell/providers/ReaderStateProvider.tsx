...
//   - the app-global dictionary / add-card modal
...
/**
 * State for the app-global modal (pages 10/11 — `features/books/reader/
 * reader-modal`). Two mutually-exclusive modes share the slot:
 *
 * - `'dict'` — the modal shows the dictionary surface. The *content*
 *   (query, results, selected word) lives in `DictionaryStateProvider`;
 *   this state only controls visibility. AppShell reads `mode === 'dict'`,
 *   renders the modal, and the modal reads `useDictionaryState()`
 *   for the actual rendering.
 *
 * - `'addCard'` — the modal shows the "add this word to a deck" form,
 ...
 *   consumers are intentional (modal = inline finish, pendingCard =
 *   navigate to finish).
 *
...
export type ReaderModalState =
  ...
  | {
       ...
       *  because the modal needs it before a draft exists: it is the remount
       *  key, the query for the modal's own initial lookup, and — for a card
       ...
       *  `null`. The modal needs it twice — it seeds `runSearch`'s
       *  `readerContext`, which is what lets `contextForEntry` attach the book
       *  sentence to the right row of the modal's own dictionary, and it seeds
       ...
       *  modal (`/dictionary`, or the reader with the sidekick docked). The
       *  modal then skips its own lookup — running one would overwrite the
...
type ReaderContextValue = {
  ...
  // The app-global modal (dict lookup or add-card flow). Lives here so
  // producers can open it via `useReaderActions` without a pending-field
  // handshake.
  readerModal: ReaderModalState | null;
  setReaderModal: React.Dispatch<React.SetStateAction<ReaderModalState | null>>;

  // Dictionary sidekick — docked beside the reading surface.
  // `useReaderActions` reads `sidekickOpen` to decide whether a dict lookup
  // should open the modal or just route into the visible sidekick.
...
export function ReaderStateProvider({ children }: { children: React.ReactNode }) {
  ...
  const [readerModal, setReaderModal] = useState<ReaderModalState | null>(null);
