 ...
 * screen. Kept in the tree because the auth handoff draws it on Sign In and
 * the backend doesn't support it: there is no OAuth anywhere in
 * `backend/src/routes/auth.js`, no provider column on `users`, and no
 * callback route. Two prominent buttons that do nothing are worse than two
 * that aren't drawn, so they stay dark until there's something behind them.
 *
 * To enable: flip the flag, then wire `onStart` to the provider's authorize
 * URL. Drawn to the handoff so a flag flip reveals the right thing: the
 * `OR CONTINUE WITH` rule, then a 2-col grid of 50px white pills, 14/600 → 500
 * label (D1), the mark leading.
 */
...
const BUTTON = cn(
  PANE,
  PRESS,
  'flex h-[50px] w-full cursor-pointer items-center justify-center gap-2.5 rounded-full border-[rgb(var(--line-rgb)/0.08)]',
  'font-[family-name:var(--face-ui)] text-[14px] font-medium text-(--ink)',
  'transition-[background-color,transform] duration-120 ease-[ease] hover:bg-(--pane-strong)',
  'focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--ink)',
);

const RULE = 'h-px flex-1 bg-[rgb(var(--line-rgb)/0.10)]';
  ...
  return (
    <>
      <div className="mt-6 flex items-center gap-3.5">
        <span className={RULE} />
        <span className="font-[family-name:var(--face-mono)] text-[11px] leading-none tracking-[0.08em] uppercase text-(--ink-3)">
          or continue with
        </span>
        <span className={RULE} />
      </div>
