 ...
 *
 * `PANE` rather than a `hover:bg-*` utility: it is the list-row treatment,
    ...
    >
      <span className="min-w-0 flex-1">
        <span className="flex items-baseline gap-[9px]">
          ...
          <span className="font-[family-name:var(--face-jp)] text-[12px] text-(--ink-3)">{jp}</span>
        </span>
        <span className="mt-[3px] block text-[13px] leading-[1.45] text-(--ink-3)">
          {description}
        </span>
      </span>
      <ChevronRight aria-hidden size={18} strokeWidth={1.7} className="shrink-0 text-(--ink-3)" />
    </Link>
  );
}
