...
type LibraryViewProps = {
  /** Called when a book is clicked to be opened in the reader */
...
export default function LibraryView({ onOpenBook }: LibraryViewProps) {
  ...
  const openBook = useCallback(
    async (bookId: string) => {
      ...
      // Load the full BookRecord from IndexedDB and pass it up
      const allBooks = await getAllBooks();
      const record = allBooks.find(b => b.id === bookId);
      if (record) onOpenBook(record);
    },
    [onOpenBook],
  );
